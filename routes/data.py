...
from .schemes.data import ProcessReqest
...
async def upload_data(request: Request, project_id: str, file: UploadFile,
        ...
        file_path, file_id = data_controller.generate_unique_file_name(orig_file_name = file.filename, project_id = project_id)
        ...
        try:
            async with aiofiles.open(file_path, "wb") as f:
                ...
                loger.exception("error while uploading file")
                return JSONResponse(
                status_code = status.HTTP_400_BAD_REQUEST,
                content={
...
async def process_endpoint(request:Request, project_id: str, process_request: ProcessReqest):
    ...
    if process_request.file_id:
        ...
        project_file_ids = {
            asset_record.id : asset_record.name
        }
    ...
    if len(project_file_ids) == 0:
        return JSONResponse(
            status_code = status.HTTP_400_BAD_REQUEST,
            content = {
                "signal": ResponseSignal.RESPONSE_SIGNAL_ERROR.value,
            }
        )
    ...
    if do_reset == 1:
        _ = await chunk_model.delete_chunk_by_project(
    ...
    for asset_id, file_id in project_file_ids.items():
        ...
        if file_chunk == None or len(file_chunk) == 0:
            return JSONResponse(
                status_code = status.HTTP_400_BAD_REQUEST,
                content = {
        ...
        no_records+= await chunk_model.insert_many_chunks(chunks=file_chunk_recods)
        no_files+=1
    return JSONResponse(
        content = {
            "signal": ResponseSignal.PROCESSING_SUCCES.value,
            "inserted_chunk": no_records,
